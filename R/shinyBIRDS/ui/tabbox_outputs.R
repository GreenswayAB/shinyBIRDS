tagList(
  tabBox(id = "outputs", width = 9,
        shiny::tabPanel(tagList(shiny::icon("globe"),"Map"), 
                  leafletOutput("map", height = "83.3vh") #750 #"76.5vh" height = as.character(WindowH)
                  # uiOutput("mapvar")
                  
         ),
        shiny::tabPanel(tagList(shiny::icon("table"),"Data"),
            fluidRow(
              tabBox(id = "data-typess", width = 12,
              shiny::tabPanel(tagList(icon("binoculars"),"PBD"),
                  fluidRow(  
                      DT::dataTableOutput("TablePBD", width = "90%")
                    )
              ),
              shiny::tabPanel(tagList(icon("sitemap"),"Organised"),
                      ## explore visits
              ...
              shiny::tabPanel(tagList(icon("chart-line"),"Summarised"),
                       
              ),
              shiny::tabPanel(tagList(icon("file-export"),"For export"),
